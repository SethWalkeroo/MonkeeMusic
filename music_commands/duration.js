...
module.exports = {
	...
	async execute(message, args) {
		...
		if (!serverQueue) return await message.channel.send('There are currently no songs playing.');

		// if no argument is specified, get the duration of the current song.
		if (!args[0]) {
			if (!serverQueue.songs.length) {
				return await message.channel.send('There is no song playing!');
			}
		...
		if (args[0] === 'queue') {
			...
			if (!totalDuration) {
				return await message.channel.send('The currenet queue has no duration!');
			}
		...
		if (playlistName) {
			...
			return await message.channel.send('The specified playlist does not exist! :worried:');
		}
	},
};